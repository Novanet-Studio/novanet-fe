  ...
  <section :id="props.content.name ? props.content.name : ''" :data-color="props.content.dataColor" :data-emblem-color="emblemModifierSource[props.content.name] || props.content.dataColor
    ...
    <div v-else-if="formattedArticles.length > 0">
      <h1 :class="[`main__title pb-8`, props.content.titleColor]">
        <Motion :initial="animations.mainTitle.initial" :animate="hasBeenAnimated(props.content.name)
          ...
          <NuxtLink :to="article.cta_route" class="flex flex-col gap-2">
            <div class="text-left">
              <p class="text-gray-500 text-base">
                <Motion :initial="animations.description.initial" :animate="hasBeenAnimated(props.content.name)
                  ...
                  " :transition="{ ...animations.description.transition }">{{ article.date }}
                </Motion>
              </p>
            ...
            <div
              ...
              <Motion :initial="animations.mainImage.initial" :animate="hasBeenAnimated(props.content.name)
                ...
                <NuxtImg v-if="article.portrait" provider="cloudinary" :src="article.portrait"
                  :alt="`Imagen del artículo ${article.title}`" class="w-full h-full object-cover" loading="lazy" />
