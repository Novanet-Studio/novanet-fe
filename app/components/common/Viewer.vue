        ...
        >
          <NuxtLink
            ...
            :class="[
              ...
              content.buttonClass ||
                'hover:bg-opacity-80 bg-azure text-oxfordBlue',
            ]"
          >
            Ver proyecto
            <span>&nbsp;→</span>
      ...
      >
        <div class="flex flex-col pb-12">
          <div class="max-h-[50vh] overflow-hidden">
            <div
              ref="carouselTrackEl"
              class="flex gap-8 transition-transform duration-500 ease-in-out md:max-h-[30vh] lg:max-h-[50vh]"
              :style="carouselStyle"
            >
              <div
                ...
                :key="item.cta_route"
                :class="[
                  'h-full flex-shrink-0 overflow-hidden shadow-xl',
              ...
              >
                <NuxtLink v-if="item === currentItem" :to="item.cta_route">
                  <NuxtImg
                    ...
                    :src="item.portrait"
                    :alt="item.title"
                    class="h-auto w-full object-cover md:max-h-[30vh] lg:max-h-[50vh]"
                  />
                </NuxtLink>
                ...
                <NuxtImg
                  ...
                  :src="item.portrait"
                  :alt="item.title"
                  class="h-auto w-full cursor-pointer object-cover md:max-h-[30vh] lg:max-h-[50vh]"
                />
              </div>
            </div>
          </div>

          <div v-if="totalItems > 1" class="mt-8 flex items-center gap-4">
            <button
              @click="prevItem"
              :class="[
                'hover:bg-white hover:bg-opacity-10 flex h-10 w-10 items-center justify-center rounded-full border-2 transition-all duration-300 md:h-12 md:w-12',
