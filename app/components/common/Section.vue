...
const { isAnimated, scrollToSection } = useSectionObserver(props.content);
</script>
...
<template>
  <section :id="item.name ? item.name : ''" v-for="(item, index) in props.content" :key="index"
    ...
    <div :class="[
      ...
      <!-- CTA Navegación -->
      <Motion v-if="item.navButtonBack" :key="`cta-${index}`" :initial="animations.cta.initial" :animate="isAnimated(index) ? animations.cta.animate : animations.cta.initial
      ...
      <!-- Título -->
      <Motion :key="`mainTitle-${index}`" :initial="animations.mainTitle.initial" :animate="isAnimated(index)
      ...
      <!-- Descripción -->
      <Motion v-if="item.description" :key="`desc-${index}`" :initial="animations.description.initial" :animate="isAnimated(index)
      ...
      <!-- CTA Primario -->
      <Motion v-if="item.buttonText" :key="`cta-${index}`" :initial="animations.cta.initial" :animate="isAnimated(index) ? animations.cta.animate : animations.cta.initial
        " :transition="{ ...animations.cta.transition }">
      ...
      <!-- Formulario -->
      <Motion v-if="item.showForm" :key="`form-${index}`" :initial="animations.ThirdElement.initial" :animate="isAnimated(index)
      ...
      <!--  Tabs-->
      <Motion v-if="item.tabs" :key="`tabs-${index}`" :initial="animations.ThirdElement.initial" :animate="isAnimated(index)
        ? animations.ThirdElement.animate
        : animations.ThirdElement.initial
        " :transition="{ ...animations.ThirdElement.transition }">
        <div class="tabs">
      ...
      <!-- Lista -->
      <Motion v-if="item.list" :key="`list-${index}`" :initial="animations.ThirdElement.initial" :animate="isAnimated(index)
        ? animations.ThirdElement.animate
        : animations.ThirdElement.initial
        " :transition="{ ...animations.ThirdElement.transition }">
        <div class="lista">
    ...
    <div v-if="item.HeroImage" :class="[
      ...
      <Motion v-if="item.HeroImage" :key="`mainImage-${index}`" :initial="animations.mainImage.initial" :animate="isAnimated(index)
    ...
    <div v-if="item.serviceImage" class="flex xl:flex-col xl:pr-10 xl:w-[33.3333333%] xl:justify-center xl:h-full">
      <Motion v-if="item.serviceImage" :key="`serviceImage-${index}`" :initial="animations.mainImage.initial" :animate="isAnimated(index)
        ? animations.mainImage.animate
        : animations.mainImage.initial
        " :transition="{ ...animations.mainImage.transition }">
