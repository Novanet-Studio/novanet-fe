  ...
  <section :id="props.content.name ? props.content.name : ''" :data-color="props.content.dataColor"
    ...
    <div v-else-if="tabs.length > 0" class="w-full overflow-hidden">
      <nav class="flex gap-x-8" aria-label="Tabs">
        <button v-for="tab in tabs" :key="tab.key" :ref="el => (tabRefs[tab.key] = el as HTMLElement)"
          @click="activeTabKey = tab.key" :class="[
            'pb-3 text-azure text-left transition-colors duration-200 ease-out focus:outline-none',
      ...
      <div v-if="activePanel" class="mt-4 space-y-4">
        <p class="max-w-4xl text-base">
          <span class="font-bold text-oxfordBlue">{{ activePanel.label }}:</span>
